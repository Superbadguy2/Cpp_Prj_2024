// Lyf_FFT_iFFT.cpp : 用于实现fft和ifft算法的C++代码
#include "Lyf_FFT_iFFT.h"
...
std::vector<Sample_Data::cpd> Sample_Data::Recursive_fft1(std::vector<cpd>& x){
    ...
    for(int i = 0; i < x.size()/2; i++) {// 将x分成两个子序列X_e和X_o
        X_e[i] = x[2*i];
        X_o[i] = x[2*i+1];
    }
    // 对子序列X_e和X_o分别进行fft
...
std::vector<Sample_Data::cpd> Sample_Data::Data_UpPadding(std::vector<cpd> &x)
{
    ...
    for(int i = 0; i < Sample_Data::GetFillSampleDataLength(); i++){
